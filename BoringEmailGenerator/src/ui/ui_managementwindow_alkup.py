...
class ManagementWindow(Toplevel):
    ...
    def _draw_group_placeholder_label(self, frame_id, message_id):
            info_label = ttk.Label(master=self.group_message_frames[frame_id], text="")
            info_label.grid(row=message_id*2+1, column=0, columnspan=2, sticky=(
            constants.E, constants.W), padx=2, pady=2)
    
    def _draw_delete_message_button(self, frame_id, message_id):
        group_name_button_style = ttk.Style()
        group_name_button_style.theme_use('default')
        group_name_button_style.configure('Group.TButton', padding=2, relief="flat", font=(
            'Georgia', 12, 'bold'), background='#d966ff', foreground='black')
        group_name_button_style.map('Group.TButton', background=[('active', '#ff66ff'), (
            'disabled', '#8c66ff')], foreground=[('pressed', '#660066'), ('active', 'black')])
        group_name_button = ttk.Button(master=self.group_message_frames[frame_id], text=f"Delete", style='Group.TButton',
                                   command=lambda:self.handle_delete_message_button(frame_id, message_id))
        group_name_button.grid(row=message_id*2+1, column=2, columnspan=1, padx=5, pady=5, sticky=(constants.E, constants.W))
    ...
    def _draw_save_message_button(self, frame_id, message_id):
        ...
        group_name_button_style.map('Group.TButton', background=[('active', '#ff66ff'), (
            'disabled', '#8c66ff')], foreground=[('pressed', '#660066'), ('active', 'black')])
        group_name_button = ttk.Button(master=self.group_message_frames[frame_id], text=f"Save", style='Group.TButton',
                                   command=lambda:self.handle_save_message_button(frame_id, message_id))
        group_name_button.grid(row=message_id*2+1, column=3, columnspan=1, padx=5, pady=5, sticky=(constants.E, constants.W))
